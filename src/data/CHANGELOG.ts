...
export const CHANGELOG: VersionChangelog[] = [
  {
    ...
    date: 'April 23, 2026',
    entries: [
      { type: 'new', text: 'Broadcast Center — A high-priority communication hub for Admin to send global workstation alerts with mandatory acknowledgment.' },
      { type: 'new', text: 'Workstation Tracking — Administrators can now see a real-time audit trail of which machines and users have acknowledged a specific broadcast.' },
      { type: 'new', text: 'Acknowledgment Dynamic Islands — Premium, slide-down visual feedback when a broadcast is confirmed across the network.' },
      { type: 'new', text: 'Success Audio Feedback — Real-time audible "chirp" alerts when workstations respond to priority broadcasts.' },
      { type: 'new', text: 'AI Tech Assistant — You can now ask questions about finding files and folders on the indexed data. (Limited to 3/day during development).' },
      { type: 'new', text: 'Clients Page — A dedicated module for managing engineering clients and contacts with category filtering.' },
      { type: 'new', text: 'Inverted Paper Mode — The Material Calculator now intelligently switches its aesthetic based on your active theme.' },
      { type: 'improvement', text: 'Modernized Calculator UI — Complete visual overhaul with Dark, Light, and Void theme support.' },
      ...
      { type: 'improvement', text: 'Ultra-Responsive TitleBar — Hardened navigation layout to ensure accessibility at narrow widths.' },
      { type: 'improvement', text: 'Code & Logic Cleanup — Streamlined material spec parsing and removed legacy code blocks for peak performance.' },
    ],
  },
  {
    version: '3.6.7',

...
async def join_doc(sid: str, data: dict):
    ...
    async with AsyncSessionLocal() as session:
        quot = await quotation_repo.get(session, q_id)
        ...
        if not quot.is_active:
            await quotation_repo.update(session, id=q_id, obj_in={"is_active": True})
...
# ─── Internal Helpers ────────────────────────────────────────────────────────
# Helpers moved to QuotationService
...
async def update_field(sid: str, data: dict):
    ...
    if full_state:
        from db.database import AsyncSessionLocal
        async with AsyncSessionLocal() as session:
            await QuotationService.sync_metadata(q_id, full_state, session)
...
async def trigger_snapshot(sid: str, data: dict):
    ...
    from db.database import AsyncSessionLocal
    async with AsyncSessionLocal() as session:
        await QuotationService.create_snapshot(session, q_id, full_state, label, author)
    ...
    for q_id, users in list(_active_users.items()):
        if sid in users:
            ...
            if not users:
                ...
                try:
                    async with AsyncSessionLocal() as session:
                        await quotation_repo.deactivate(session, q_id)
                except Exception as e:

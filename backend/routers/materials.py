...
async def create_material(data: MaterialCreate, db: AsyncSession = Depends(get_db)):
    ...
    return {"id": item.id, "englishName": item.english_name, "japaneseName": item.japanese_name}


@router.patch("/{item_id}", dependencies=[Depends(require_role(["admin", "it"]))])
async def update_material(item_id: int, data: MaterialUpdate, db: AsyncSession = Depends(get_db)):
